...
/* REPETIER_H CONFIGURATION
 *  ARDUINO MEGA + RAMPS 1.4
 *  Viki LCD
 *  Single Extruder + HeatBed
 */
 ...
 *  
4.5 
Jam Detection Enabled Pin: 23

4.4  
Heat bed always on issue acknowledged
heat bed issue resolved ( swapped Ramps control board)
max tempt -> 120 (220)

4.3
heat Bed 400W 12V with  NTC 100K thermistor
heat bed manager changed to val 3

4.2
yaxis pulley diameter changed-> steps changed
heated bed  with thermistor: NTC 100K thermistor  Beta 25/50 3950K ( type 14?? )
thermistor on ext_2
PS ON implemented

4.1
Z Home fixed // check end stop remap at header of section
SD fixed



4.0 regenerated repetier 0.9.2
Z HOME DOES NOT WORK!!!?!?!!
EXT0_JAM_PIN -1//27 DISABLED
viki LCD setup
resolutions, feedrates, 
sd still causing issues
print fan on D23
extruder cooler at fan pin
jam detection on pin 27
SD card detect on sd card detect
case Light pin on pin 29

3.5
FAN autocool setting?
SD card fix?

3.4
VIKI LCD ENABLED
SD CARD DETECT SET TO 1
SD SUPPORT SET TO 1
...
#define HEATED_BED_HEATER_PIN HEATER_1_PIN  // NOT SURE IF THIS IS RIGHT???
...
#define UI_PRINTER_NAME "HH_BOT v2.0"
#define UI_PRINTER_COMPANY "HH_Concepts"
